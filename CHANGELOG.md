...
- **Compaction transcript continuity**: When context compaction rolls a conversation into a child session, Hermes now persists the rebuilt compacted transcript from the start of that new session instead of incorrectly skipping writes based on the parent session's history length. Resume/gateway follow-up turns keep the compacted summary and turn order intact.
...
- **Codex auxiliary vision model**: Codex-backed auxiliary tasks now default to `gpt-5.4` instead of `gpt-5.2-codex`, so browser-side image auto-analysis and other vision flows stay aligned with the current Codex model path.
- **Codex auxiliary stream recovery**: Codex-backed auxiliary calls now recover text from streamed `Responses` deltas/done events and honor per-call timeouts, which fixes empty-content vision failures and prevents `vision_analyze` / `web_extract` from ignoring the configured auxiliary timeout budget.
...
- **Discord thread-only context bundles**: On Discord channel/thread pages, the sidepanel now exposes the active conversation as its own `Discord thread` bundle plus `Discord thread images`, so you can inject just the support thread text/screenshots without dragging in the full app chrome, channel list, or other thread titles by default.
...
- **Bounded web extraction waits**: Firecrawl-backed `web_extract` and `web_crawl` now enforce `web.request_timeout`, and the optional post-extraction summarizer now honors `auxiliary.web_extract.timeout` plus a small `auxiliary.web_extract.max_retries` budget so stalled scrape/LLM phases fail fast instead of sitting in gateway progress for many minutes after Firecrawl already finished.
- **Small extract fast-path**: `web_extract` now skips the optional LLM summarizer by default for modest pages (under ~12k characters), which keeps support-thread pastes and similar small docs on the fast raw-extract path instead of spending ~tens of seconds in an extra auxiliary-model roundtrip.
