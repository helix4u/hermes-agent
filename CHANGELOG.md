...
- **Browser STT model defaults**: The browser options page now uses dropdowns for local/OpenAI STT models and preserves sane defaults (`base`, `whisper-1`) instead of saving empty strings that could break Discord/browser voice transcription.
...
- **Tool budget note cleanup**: The transient tool-budget hint is now injected as a short system message instead of a synthetic user message, and it only reports whether tool calls are usable plus the remaining `x/y` tool-call budget.
