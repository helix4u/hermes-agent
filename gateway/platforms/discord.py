...
class DiscordAdapter(BasePlatformAdapter):
    ...
    async def _set_online_presence(self) -> None:
        """Force an explicit online presence so Discord shows a green status dot."""
        if not self._client:
            return
        try:
            status_map = {
            ...
            status_key = (os.getenv("DISCORD_PRESENCE_STATUS", "online") or "online").strip().lower()
            status = status_map.get(status_key, discord.Status.online)

            activity_text = (os.getenv("DISCORD_PRESENCE_TEXT", "Hermes Gateway") or "").strip()
            activity_type_key = (os.getenv("DISCORD_PRESENCE_TYPE", "watching") or "watching").strip().lower()
            activity_type = activity_map.get(activity_type_key, discord.ActivityType.watching)

            activity = None
            if activity_text:
                if activity_type == discord.ActivityType.streaming:
                    stream_url = (
                        os.getenv("DISCORD_PRESENCE_STREAM_URL")
                        or "https://www.twitch.tv/discord"
                    )
            ...
            await self._client.change_presence(status=status, activity=activity)
            logger.info(
                "[discord] presence set: status=%s activity_type=%s activity=%s",
                status_key,
                activity_type_key,
                activity_text or "<none>",
            )
        except Exception as e:
            logger.warning("[discord] failed to set presence: %s", e)

    async def _presence_refresh_loop(self) -> None:
        """Periodically re-assert presence in case Discord reconnects reset it."""
        while self._client and not self._client.is_closed():
            try:
                interval = int(os.getenv("DISCORD_PRESENCE_REFRESH_SECONDS", "90"))
            except Exception:
                interval = 90
    ...
    def _ensure_presence_task(self) -> None:
        """Start one refresh task per adapter lifecycle."""
    ...
    async def create_tool_log_thread(
        self,
        chat_id: str,
        seed_message_id: str,
        title: str,
    ) -> Optional[str]:
        """Create a thread from a message for detailed per-run tool logs."""
        if not self._client:
            return None
        try:
            channel = self._client.get_channel(int(chat_id))
            ...
            # Discord DMs do not support threads.
            if isinstance(channel, discord.DMChannel):
                return None
            if isinstance(channel, discord.Thread):
                return str(channel.id)

            seed_msg = await channel.fetch_message(int(seed_message_id))
            if not seed_msg:
                return None

            thread_name = (title or "Hermes Tool Log").strip()[:100]
            ...
            logger.debug("Discord create_tool_log_thread failed: %s", e)
            return None
