...
class TestSendToPlatformChunking:
    def test_long_message_is_chunked(self):
        """Discord detached sends chunk at the embed-description limit."""
        send = AsyncMock(return_value={"success": True, "message_id": "1"})
        long_msg = "word " * 1800  # ~9000 chars, well over Discord's embed limit
        ...
        for call in send.await_args_list:
            assert len(call.args[2]) <= 4000  # each embed chunk fits the limit

    def test_discord_rest_sender_uses_embed_payload(self, monkeypatch):
        captured = {}
        ...
        result = asyncio.run(_send_discord("tok", "1234", "hello discord"))
