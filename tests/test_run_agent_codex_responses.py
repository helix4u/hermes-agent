...
def test_run_codex_stream_backfills_output_from_done_events(monkeypatch):
    agent = _build_agent(monkeypatch)
    ...
    assert response.output[0].content[0].text == "done event text"


def test_run_codex_stream_synthesizes_output_from_text_deltas(monkeypatch):
    agent = _build_agent(monkeypatch)
    agent.client = SimpleNamespace(
        responses=SimpleNamespace(
            stream=lambda **kwargs: _FakeResponsesStream(
