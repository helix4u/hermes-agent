...
- Exposed live tool-use runtime status to the model as a transient per-call note so it can see whether tool calls are still usable on the current response, how many follow-up iterations remain, what the soft tool-call cap is, and whether that soft budget is exhausted without polluting cached system prompts.
